...
This the first project since I take the CS61A course. Although this project, a simple logic game with the concept of probablity and game theory, is simple(Because I don't need to write the GUI and the main structure of this game), I learned a lot in the process of doing this project. 

- When you want to write a relatively big program, such as the Hog game, you should **seperate it into small partitions** for simpler implement and less mistakes. 

- Write small functions, ensuring that they **only finish one task** when called, and assemble them into a big program. 

- **Debuging is as much important as coding**. There are several kinds of method to debug, sometimes we need to more systematic and complex methods to debug our code. **Be patient and scrupulous**.

I have upload the code of the Hog project on Github. Coding is really interesting.

<h2 id="5">Leture 5</h2>

I found that take down everything is not a effective way to learn how to write code, so I will only take down knowledge that is strange to me or mistakes I made, in the following chapters.
...
- There are subtle differences between built-in type ```float``` and ```int```. ```float``` can be considered as **the approximation of the real value**. Be careful when using ```float``` to compare with other value.

- **Abstraction barrier makes program easier to read and modify**. When the part of a program that could use higher level function uses lower level function, an **abstraction  barrier violation** occurs.